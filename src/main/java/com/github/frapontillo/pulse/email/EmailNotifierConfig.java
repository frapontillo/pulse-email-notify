 ...
 * <li>body_success: the body of a successful report email (references to {{NAME}} will be replaced
 * with the process
 * name)</li>
 * <li>body_error: the body of an error report email (references to {{NAME}} will be replaced with
 * the process
 * name)</li>
...
public class EmailNotifierConfig implements IPluginConfig<EmailNotifierConfig> {
    ...
    private static final String DEFAULT_BODY_SUCCESS =
            "The pipeline \"{{NAME}}\" has completed successfully.";
    ...
    @Override public EmailNotifierConfig buildFromJsonElement(JsonElement json) {
        EmailNotifierConfig config =
                PluginConfigHelper.buildFromJson(json, EmailNotifierConfig.class);
